...
import
  json_schema_import/private/
    [parse, gen, util, equality, bin, sax, empty, validate, json_hooks, cache]

export
  JsonSchemaConfig, UrlResolver, json, jsonutils, equality, bin, sax, empty, validate,
  json_hooks
...
proc parseJsonSchema(
    schema: JsonNode, conf: JsonSchemaConfig
...
proc parseJsonSchema(
    schema: string, conf: JsonSchemaConfig
): GeneratedOutput {.compileTime.} =
  ## Parses a json schema from a string
  parseJsonSchema(schema.parseJson, conf)

macro realImportJsonSchema(
    rootDir, path: static string, conf: static JsonSchemaConfig
) =
