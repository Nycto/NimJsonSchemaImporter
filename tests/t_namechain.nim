...
suite "Name chaining and generation":
  ...
  test "Single added name":
    check(
      take(rootName("Foo").add("bar"), 4) ==
        @["PreBar", "PreFooBar", "PreFooBar2", "PreFooBar3"]
    )

  test "Triple added names":
    check(
      take(rootName("Foo").add("bar").add("baz"), 4) ==
        @["PreBaz", "PreBarBaz", "PreFooBarBaz", "PreFooBarBaz2"]
    )

  test "Categorized name":
    check(
      take(rootName("Foo").categorize("Union"), 4) ==
        @["PreFoo", "PreFooUnion", "PreFooUnion2", "PreFooUnion3"]
    )

  test "Multiple Categorized names":
    check(
      take(rootName("Foo").categorize("Union").add("bar").categorize("Obj"), 5) ==
        @[
          "PreBar", "PreBarObj", "PreFooBarObj", "PreFooBarObjUnion",
          "PreFooBarObjUnion2",
        ]
    )
