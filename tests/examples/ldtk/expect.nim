...
proc fromJsonHook*(target: var `TestEntityInstance`; source: JsonNode) =
  ...
  target.`smartColor` = jsonTo(source{"__smartColor"},
                               typeof(target.`smartColor`))
...
proc fromJsonHook*(target: var `TestLayerInstance`; source: JsonNode) =
  ...
  target.`layerDefUid` = jsonTo(source{"layerDefUid"},
                                typeof(target.`layerDefUid`))
...
proc fromJsonHook*(target: var `TestLevel`; source: JsonNode) =
  ...
  target.`neighbours` = jsonTo(source{"__neighbours"},
...
proc fromJsonHook*(target: var `TestFieldDef`; source: JsonNode) =
  ...
  target.`allowedRefs` = jsonTo(source{"allowedRefs"},
                                typeof(target.`allowedRefs`))
...
proc fromJsonHook*(target: var `TestEnumTagValue`; source: JsonNode) =
  ...
  target.`enumValueId` = jsonTo(source{"enumValueId"},
                                typeof(target.`enumValueId`))
...
proc fromJsonHook*(target: var `TestTestEntityDef_limitBehavior`;
                   source: JsonNode) =
  target = case getStr(source)
  of "PreventAdding":
...
proc fromJsonHook*(target: var `TestEntityDef`; source: JsonNode) =
  ...
  target.`tileOpacity` = jsonTo(source{"tileOpacity"},
                                typeof(target.`tileOpacity`))
  assert("color" in source,
  ...
  target.`fillOpacity` = jsonTo(source{"fillOpacity"},
                                typeof(target.`fillOpacity`))
...
proc fromJsonHook*(target: var `TestAutoRuleDef`; source: JsonNode) =
  ...
  target.`tileYOffset` = jsonTo(source{"tileYOffset"},
                                typeof(target.`tileYOffset`))
  ...
  target.`invalidated` = jsonTo(source{"invalidated"},
                                typeof(target.`invalidated`))
  ...
  target.`tileXOffset` = jsonTo(source{"tileXOffset"},
                                typeof(target.`tileXOffset`))
...
proc fromJsonHook*(target: var `TestLdtkJsonRoot`; source: JsonNode) =
  ...
  target.`backupLimit` = jsonTo(source{"backupLimit"},
                                typeof(target.`backupLimit`))
  ...
  target.`jsonVersion` = jsonTo(source{"jsonVersion"},
                                typeof(target.`jsonVersion`))
