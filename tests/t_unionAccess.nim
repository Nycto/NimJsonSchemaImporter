      ...
      "properties": {
        "entries": {
          "anyOf": [
            ...
            {"items": {"$ref": "#/definitions/Person"}, "type": "array"},
          ]
        }
      },
    },
...
suite "Interacting with unions":
  ...
  test "Creating a union of a seq":
    let people = People(
      entries: forUnion @[Person(name: "Jack", age: 41), Person(name: "Jill", age: 42)]
    )

    check(
      $people.toJson ==
        """{"entries":[{"name":"Jack","age":41},{"name":"Jill","age":42}]}"""
    )
  ...
  test "Creating a union of a map":
    let people = People(
      entries: forUnion(
        toOrderedTable {
          "a": Person(name: "Jack", age: 41), "b": Person(name: "Jill", age: 42)
        }
      )
    )
    ...
    check(
      $people.toJson ==
        """{"entries":{"a":{"name":"Jack","age":41},"b":{"name":"Jill","age":42}}}"""
    )

  test "Converter for creating a union":
    let people =
      People(entries: @[Person(name: "Jack", age: 41), Person(name: "Jill", age: 42)])
